...
typedef Validator<IN extends Object?, OUT extends Object?>
    = BaseElementaryValidator<IN, OUT>;

/// Provides utility methods for creating various [FormFieldValidator]s.
/// This is the prototype for the refactored version of the FormFieldValidator
class ValidatorBuilder {
  // Null handling validators
  /// Builds a validator that requires the field to have a non-empty value.
  ///
  /// ## Parameters:
  ...
  /// may be used to give the alternative when the field is provided.
  ///
  /// ## Parameters:
  /// - [errorText] The error message when the value is empty.
  static Validator<T?, T?> optional<T extends Object>({
    required List<BaseElementaryValidator<T, dynamic>> otherwise,
    String? errorText,
  }) {
    assert(
      otherwise.isNotEmpty,
      'There must be an alternative condition for when the field is provided.',
    );
    return IsNotRequiredElementaryValidator<T>(
      errorText: errorText,
      otherwise: otherwise,
    );
  }
  ...
  static Validator<String, String> hasLowercaseChars({
    int atLeast = 1,
    RegExp? regex,
    String? errorText,
  }) =>
