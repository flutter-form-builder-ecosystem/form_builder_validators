...
void main() {
  group('Validator: matchesAllowedExtensionsValidator', () {
    ...
    testCases = <({List<String> extensions, bool isValid, String userInput})>[
    ...
    for (final ({List<String> extensions, bool isValid, String userInput})
        testCase
        in testCases) {
      test(
        ...
        () {
          final String errorMsg = FormBuilderLocalizations.current
              .fileExtensionErrorText(testCase.extensions.join(', '));
          final Validator<String> v = matchesAllowedExtensions(
            testCase.extensions,
          );

          expect(v(testCase.userInput), testCase.isValid ? isNull : errorMsg);
        },
      );
    }
    ...
    test(
      ...
      () {
        ...
        final String errorMsg = FormBuilderLocalizations.current
            .fileExtensionErrorText(extensions.join(', '));
        final Validator<String> v = matchesAllowedExtensions(
          extensions,
          caseSensitive: false,
        );
    ...
    test(
      'should be immutable even when the input list of extensions change',
      () {
