...
void main() {
  group('Validator: password', () {
    group('Validations with default error message', () {
      final List<({String? expectedOutput, String password})> testCases =
          <({String? expectedOutput, String password})>[
        // Valid one
        ...
        (
          password: '',
          expectedOutput:
              FormBuilderLocalizations.current.minLengthErrorText(16)
        ),
        (
          password:
              'this string has more than 32 characters, which is not allowed as a password by default',
          expectedOutput:
              FormBuilderLocalizations.current.maxLengthErrorText(32)
        ),
        (
        ...
        (
          ...
          expectedOutput:
              FormBuilderLocalizations.current.containsNumberErrorText(1)
        ),
        (
    ...
    group('Validations with custom', () {
      ...
      final List<({bool fails, String password})> testCases =
          <({bool fails, String password})>[
        // Valid one
      ...
      final Validator<String> validator =
          u.password(passwordMsg: (_) => customErrorMessage);
    ...
    test('Should throw argument error if any of the arguments are invalid', () {
      ...
      expect(
          () => u.password(minLength: 12, maxLength: 3), throwsArgumentError);
    });
  });
}
