...
void main() {
  ...
  group('Validator: isTrue', () {
    group('Validations with default error message', () {
      ...
      for (final (input: Object input, isValid: bool isValid) in testCases) {
        ...
        test(
          ...
          () {
            expect(v.isTrue()(input), equals(expectedReturnValue));
          },
        );
      }
    });
    ...
    test(
      ...
      () {
        expect(
          v.isTrue(caseSensitive: true)('tRue'),
          FormBuilderLocalizations.current.mustBeTrueErrorText,
        );
      },
    );
    ...
    test(
      'should return the custom error message when the value is not true',
      () {
        // Arrange
        final v.Validator<String> validator = v.isTrue(
          isTrueMsg: (_) => customErrorMessage,
        );
