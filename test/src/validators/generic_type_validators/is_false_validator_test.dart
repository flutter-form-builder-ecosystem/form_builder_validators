...
void main() {
  ...
  group('Validator: isFalse', () {
    group('Validations with default error message', () {
      ...
      for (final (input: Object input, isValid: bool isValid) in testCases) {
        ...
        test(
          ...
          () {
            expect(v.isFalse()(input), equals(expectedReturnValue));
          },
        );
      }
    });
    ...
    test(
      ...
      () {
        expect(
          v.isFalse(caseSensitive: true)('False'),
          FormBuilderLocalizations.current.mustBeFalseErrorText,
        );
      },
    );
    ...
    test(
      'should return the custom error message when the value is not false',
      () {
        // Arrange
        final v.Validator<String> validator = v.isFalse(
          isFalseMsg: (_) => customErrorMessage,
        );
