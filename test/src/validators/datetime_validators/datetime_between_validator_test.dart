...
void main() {
  ...
  group('Validator: dateTimeBetween', () {
    test('Validation for the range 1994 and 1997', () {
      ...
      final DateTime afterRight1Second = rightReference.add(
        const Duration(seconds: 1),
      );
      final DateTime beforeRight1Second = rightReference.subtract(
        const Duration(seconds: 1),
      );
      final DateTime between = DateTime(1995);
      final DateTime beforeLeft = DateTime(199);
      final DateTime afterLeft1Micro = leftReference.add(
        const Duration(microseconds: 1),
      );
      final DateTime beforeLeft1Micro = leftReference.subtract(
        const Duration(microseconds: 1),
      );
      ...
      expect(v(leftEq), errorMsg, reason: 'Should return error against 1994');
      expect(v(rightEq), errorMsg, reason: 'Should return error against 1997');
      expect(
        v(afterRight),
        errorMsg,
      ...
      expect(v(between), isNull, reason: 'Should return null against 1995');
      expect(
        v(afterLeft1Micro),
        isNull,
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          isNull,
          reason:
              'Should return null against a datetime between the references',
        );
        expect(
          v(rightEq),
          errorMsg,
    ...
    test('Should return a custom message after validating', () {
      ...
      final Validator<DateTime> v = betweenDateTime(
        leftReference,
        rightReference,
        betweenDateTimeMsg: (_, _, _) => errorMsg,
      );
    ...
    test(
      ...
      () {
        expect(
          () => betweenDateTime(
            DateTime(1990, 12, 23, 20),
            DateTime(1990, 12, 22, 20),
          ),
