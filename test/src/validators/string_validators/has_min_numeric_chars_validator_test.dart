...
void main() {
  ...
  group('Validator: hasMinNumericCharsValidator', () {
    group('Validations with default error message', () {
      ...
      for (final (
        ...
        final String? expectedReturnValue = isValid
            ? null
            : FormBuilderLocalizations.current.containsNumberErrorText(
                minValue ?? 1,
              );
        test(
      ...
      test(
        ...
        () {
          final Validator<String> validator = hasMinNumericChars(
            hasMinNumericCharsMsg: (_, _) => customErrorMessage,
          );
    ...
    test(
      'Should pass when abcdef and ABCDEF is also considered numeric digits',
      () {
        const String s = '123aBc ijklm*';

        expect(
          hasMinNumericChars(min: 6)(s),
        ...
        expect(
          hasMinNumericChars(
            ...
            customNumericCounter: (String v) =>
                RegExp('[0-9a-fA-F]').allMatches(v).length,
          )(s),
          isNull,
        );
      },
